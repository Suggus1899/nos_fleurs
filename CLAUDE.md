...
- `src/lib/products.ts` — `getProducts()`/`saveProducts()` read and write `data/products.json` (via `node:fs`, server-only), plus `getProductBySlug`, `slugify`, `Occasion` (now a plain `string`, not a fixed union — see `lib/occasions.ts`). This is the only data store — no CMS/DB. Each product's `image` is a placeholder photo hotlinked from Pexels — swap for real photography when available.
- `src/lib/occasions.ts` — `getOccasions()`/`saveOccasions()` read/write `data/occasions.json`, the admin-editable category list (`/admin/categorias`). A category can't be deleted while a product still references it (`deleteOccasion` in `src/app/admin/actions.ts` checks and redirects with `?error=in-use`).
...
Product/gallery images use `next/image`. The 5 seed products (`data/products.json`) point at hotlinked Pexels URLs (`images.pexels.com`, allowed via `images.remotePatterns` in `next.config.ts`) — if you add images from a new remote host, add its hostname there too. Images uploaded through the admin panel are local paths under `/uploads/products/` and need no remote pattern entry.

- `src/lib/uploads.ts` — `saveUploadedImage(file)` writes to `public/uploads/products/<uuid>.<ext>` (via `node:fs/promises`) and returns the public path stored as the product's `image`; `isValidImageFile` checks type (jpeg/png/webp) and size (≤5 MB). Same local-filesystem caveat as `data/products.json` — see ponytail note below.
- Uploaded files under `public/uploads/` are committed to git (not gitignored) so the site doesn't lose product photos on a fresh clone. Expect the repo to grow with real usage; move to object storage before that matters.

## Admin panel

`/admin` lets a single hardcoded admin edit the catalog (create/edit/delete products, all fields, including uploading a photo). Credentials come from `ADMIN_USERNAME`/`ADMIN_PASSWORD` env vars — set in `.env.local` (gitignored, never commit real values); `.env.example` documents the two required vars. There is no hardcoded fallback in code on purpose, so the real password never ends up in the (public) git history.
...
- `src/app/admin/actions.ts` — all mutations (`login`, `logout`, `createProduct`, `updateProduct`, `deleteProduct`). `createProduct` requires a valid image file; `updateProduct` keeps the existing `image` if no new file is submitted. Every mutation calls `revalidatePath` for `/`, `/catalogo`, `/admin`, and the affected `/producto/[slug]` — miss one of these and that page will keep serving stale cached data after an edit.
- `src/components/admin/product-form.tsx` — the create/edit form; note `encType="multipart/form-data"` is required on the `<form>` for the file input's no-JS fallback path to work. Takes `occasions: string[]` as a prop (from `getOccasions()`) rather than importing a fixed list.
- `/admin/categorias` — list/add/delete categories. Every admin page under `/admin/productos/*` has a "← Volver" link back to `/admin`.

ponytail: auth is a single shared session cookie (no per-user sessions, no rate limiting), and both product data and uploaded photos are stored on the local filesystem (`fs`). Fine for local/dev use by one admin, but won't survive a read-only or ephemeral filesystem (e.g. Vercel) and aren't meant to scale past one admin — move to a real database (Postgres, per this workspace's stack preferences) and object storage (plus a proper auth provider) before that matters.
